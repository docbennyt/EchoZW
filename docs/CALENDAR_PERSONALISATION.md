...
EchoZW Calendar now creates a calendar subscription record for every Quick Add session.
...
- Google direct sync is available when OAuth credentials are configured.
...
Local development and EasyPanel preview use the Vite middleware. Production must mount persistent storage at `/data` or set `CALENDAR_STORE_PATH`; a later Supabase migration can replace the file store with `calendar_subscriptions` and `calendar_event_sync_records`.
