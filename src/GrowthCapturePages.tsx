      ...
      <main className="czw-growth-page">
        <section className="czw-growth-hero">
          ...
          <p>
            ...
            official source document/link, say so — that can speed up publication.
          </p>
        </section>
        <section className="czw-growth-card" aria-labelledby="request-heading">
          <div>
            <h2 id="request-heading">Request a timetable</h2>
            <p>We use this to prioritise real student demand, not to build a marketing list.</p>
          </div>
          <form onSubmit={submit}>
            <div className="czw-growth-grid">
              ...
              <Field label="Class / part / group *">
                <Input name="classGroup" required maxLength={120} placeholder="e.g. CS 1.1" />
              </Field>
              <Field label="Academic period">
                <Input name="academicPeriod" maxLength={120} placeholder="e.g. Aug Semester 2026" />
              </Field>
              <Field label="You are">
              ...
              <Field label="Source access">
                <select name="sourceAccess" defaultValue="none">
                  ...
                  <option value="class_rep">I am / can reach the Class Rep</option>
                  <option value="official_link">I have an official timetable link</option>
            ...
            <Field label="Source note or link">
              <textarea name="sourceNote" maxLength={1000} rows={4} placeholder="Optional — describe where the official timetable comes from." />
            </Field>
            <div className="czw-growth-contact">
              <h3>Optional contact</h3>
              <p>Only provide this if you want us to contact you about this request.</p>
              <div className="czw-growth-grid">
                <Field label="Name"><Input name="contactName" maxLength={120} /></Field>
                <Field label="WhatsApp / phone"><Input name="phoneE164" inputMode="tel" placeholder="+263…" /></Field>
                <Field label="Email"><Input name="email" inputMode="email" /></Field>
              </div>
            ...
            {state === "success" ? (
              <div className="czw-growth-success" role="status">Request received. We will use it to prioritise onboarding.</div>
            ) : null}
            {state === "error" ? <div className="czw-growth-error" role="alert">{message}</div> : null}
            <Button className="czw-button czw-button-primary" type="submit" disabled={state === "submitting"}>
    ...
    <PublicShell>
      <main className="czw-growth-page">
        <section className="czw-growth-hero">
          ...
          <p>Your feedback goes to the CalenderZW team. Praise is never published as a testimonial without your explicit permission and founder approval.</p>
        </section>
        <section className="czw-growth-card">
          <form onSubmit={submit}>
            <div className="czw-growth-grid">
              <Field label="Feedback type *">
                <select name="category" defaultValue="product_feedback" required>
                  <option value="timetable_problem">Timetable problem</option>
                  <option value="calendar_problem">Calendar connection problem</option>
            ...
            <Field label="Message *"><textarea name="message" required minLength={3} maxLength={4000} rows={7} /></Field>
            <Field label="Timetable slug (optional)"><Input name="publicSlug" maxLength={180} /></Field>
            <div className="czw-growth-grid">
              <Field label="Name"><Input name="contactName" maxLength={120} /></Field>
              <Field label="Email"><Input name="email" inputMode="email" /></Field>
              <Field label="WhatsApp / phone"><Input name="phoneE164" inputMode="tel" /></Field>
            </div>
            <label className="czw-growth-check"><input type="checkbox" name="consentContact" /><span>I agree CalenderZW may contact me about this feedback.</span></label>
            <label className="czw-growth-check"><input type="checkbox" name="testimonialPermission" /><span>If this is positive feedback, CalenderZW may ask to use it publicly as a testimonial.</span></label>
            {state === "success" ? <div className="czw-growth-success" role="status">Thanks — your feedback was received.</div> : null}
            {state === "error" ? <div className="czw-growth-error" role="alert">{message}</div> : null}
            <Button className="czw-button czw-button-primary" type="submit" disabled={state === "submitting"}>{state === "submitting" ? "Sending…" : "Send feedback"}</Button>
          </form>
        </section>
      </main>
    </PublicShell>
