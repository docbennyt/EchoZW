...
function parseTimetableRequest(value: unknown): TimetableRequestInsert {
  ...
  if (
    ...
    throw new Error("INVALID_INPUT");
  if (
    !["none", "class_rep", "official_link", "document", "other"].includes(
      String(sourceAccess),
    )
  )
    throw new Error("INVALID_INPUT");
...
function sendInputError(res: ServerResponse, error: unknown) {
  ...
  if (code === "PAYLOAD_TOO_LARGE") {
    sendJson(res, 413, {
      error: { code, message: "That submission is too large." },
    });
    return;
  }
...
export async function handleGrowthCaptureRequest(
  ...
  const kind =
    pathname === "/api/public/timetable-requests"
      ? "request"
  ...
  try {
    ...
    if (kind === "request") {
      const created = await createTimetableRequest(
        parseTimetableRequest(body),
        env,
      );
    ...
    if (
      ...
      sendJson(res, 500, {
        error: {
          code: "INTERNAL_ERROR",
          message: "We could not save that submission. Please try again.",
        },
      });
    }
  }
