...
describe("DR-58 academic pause migration", () => {
  it("stores pauses non-destructively at scalable scopes", () => {
    expect(migration).toContain("create table if not exists public.academic_schedule_pauses");
    expect(migration).toContain("'institution', 'programme', 'cohort', 'timetable', 'session'");
  ...
  it("supports Sim Break, Graduation, SWOT and bounded closures", () => {
    expect(migration).toContain("'sim_break', 'graduation', 'swot_week', 'holiday', 'closure', 'other'");
  ...
  it("keeps pause records server-owned and auditable", () => {
    ...
    expect(migration).toContain("alter table public.academic_schedule_pauses enable row level security");
    expect(migration).toContain("revoke all on table public.academic_schedule_pauses from anon, authenticated");
    expect(migration).toContain("grant all on table public.academic_schedule_pauses to service_role");
  });
});
