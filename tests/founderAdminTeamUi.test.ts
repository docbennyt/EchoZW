...
describe("founder and Admin Team workspace", () => {
  it(
    "shows durable founder identity and founder-only Admin invitation controls",
    () => {
  ...
  it(
    "routes operational Admins into the global shell using server permissions, not a role-string shortcut",
    () => {
      expect(pilotMvp).toContain(
